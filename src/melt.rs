use crate::{flavor::Eu4Flavor, tokens::EnvTokens, Eu4Date, Eu4Error, Eu4ErrorKind, Extraction};
use jomini::{
...
impl Melter {
    ...
    pub fn melt(&self, data: &[u8]) -> Result<(Vec<u8>, HashSet<u16>), Eu4Error> {
        self.melt_with_tokens(data, &EnvTokens)
    }
    ...
    pub fn melt_entries(
        ...
        self.melt_entries_with_tokens(metadata, gamestate, ai, &EnvTokens)
    }
...
impl<'a, 'b> Eu4Melter<'a, 'b> {
    ...
    pub fn melt<R>(&self, resolver: &R) -> Result<MeltOutput, Eu4Error>
    where
        R: TokenResolver,
    {
        melt(self, resolver)
...
pub(crate) fn melt<R>(options: &Eu4Melter, resolver: &R) -> Result<MeltOutput, Eu4Error>
where
    R: TokenResolver,
{
    let tokens = options.tape.tokens();
    let out = Vec::with_capacity(tokens.len() * 10);
    let mut unknown_tokens = HashSet::new();
